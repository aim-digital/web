  ...
  render() {
    ...
    return (
      <Modal {..._.omit(this.props, ['update', 'solution', 'create', 'destroy', 'check', 'recaptchaSiteKey', 'contact'])}
        onHide={this.onHide}
        className={`solution ${slug ? `${slug}-modal` : 'contact'}`}
        title={title}
        dek={section}
        icon={icon}
        share={share}
          ...
          <section className="content">
            {content.map((content, i) => {
              ...
              return <Fragment key={`detail-content-${i}`}>
                {Component && <Component />}
                {content.type === 'heading' && <h2 data-annotation={content.copy} className={`${content.url || content.media[0] ? 'icon' : ''}`}>{content.url || content.media[0] ? <img src={content.url || content.media[0].url} /> : <></>}{content.value}</h2>}
